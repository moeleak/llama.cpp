...
void llama_model_llada::load_arch_tensors(llama_model_loader &) {
    ...
    for (int i = 0; i < n_layer; ++i) {
        ...
        layer.attn_q_norm =
            create_tensor(tn(LLM_TENSOR_ATTN_Q_NORM, "weight", i), { n_embd_head_k }, TENSOR_NOT_REQUIRED);
        layer.attn_k_norm =
            create_tensor(tn(LLM_TENSOR_ATTN_K_NORM, "weight", i), { n_embd_head_k }, TENSOR_NOT_REQUIRED);
...
llama_model_llada::graph::graph(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
    ...
    auto * inp_attn_kv = mctx ? build_attn_inp_kv() : nullptr;
    auto * inp_attn_no_cache = mctx ? nullptr : build_attn_inp_no_cache();
